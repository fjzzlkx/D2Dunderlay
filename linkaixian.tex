...
\begin{document}
	...
	1）发射信号：如果基站为$cLink_i$分配了频率单元$r{u_i}$，那么$cLink_i$的发射端将在该频率单元上发送广播信号，广播信号携带$cLink_i$的链路ID信息。

	2）D2D链路的R端监听和汇报：所有D2D链路的R端都实时监听$\mathcal{N}_c$所有的频率单元，如果$dLink_m$的R端在频率单元$ru$上能够正确识别蜂窝链路的id，则标记该频率单元$ru$为蜂窝频率类型$\mathcal{N}_{c1}$，并记录识别的蜂窝链路的ID信息。如果$dLink_m$的R端在频率单元$ru$上检测到广播冲突，则标记该频率单元 为蜂窝频率类型$\mathcal{N}_{c2}$，否则，说明$dLink_m$的R端在频率单元 上没有收到任何信息，标记频率单元$ru$为蜂窝频率类型$\mathcal{N}_{c0}$。D2D链路的R端将检测到的$\mathcal{N}_c$所有频率单元的蜂窝频率类型和识别链路ID等信息上报给基站。

	3）

	
	
	所有D2D链路的R端都实时监听$\mathcal{N}_c$所有的频率单元，如果$dLink_m$的R端在频率单元$ru$上能够正确识别蜂窝链路的id，则标记该频率单元$ru$为蜂窝频率类型$\mathcal{N}_{c1}$，并记录识别的蜂窝链路的ID信息。如果$dLink_m$的R端在频率单元$ru$上检测到广播冲突，则标记该频率单元 为蜂窝频率类型$\mathcal{N}_{c2}$，否则，说明$dLink_m$的R端在频率单元 上没有收到任何信息，标记频率单元$ru$为蜂窝频率类型$\mathcal{N}_{c0}$。D2D链路的R端将检测到的$\mathcal{N}_c$所有频率单元的蜂窝频率类型和识别链路ID等信息上报给基站。
	
	基站统计所有的$dLink$上报的数据进行 类型干扰图更新。
	
	如果$dLink_m$上报$ru$的类型为蜂窝频率类型$\mathcal{N}_{c1}$以及链路ID对应的蜂窝链路$cLink_i$，那么此时基站更新$dLink_m$的$dNborC_m$为：
	...
	如果$dLink_m$上报$ru$的类型为蜂窝频率类型$\mathcal{N}_{c2}$，因为本文采取优先权轮播算法循环分配频率单元，该算法一个频率单元最多被两条蜂窝链路使用，而此时$dLink_m$在频率单元$ru$上检测到广播冲突，那么冲突的广播只能来自使用频率单元$ru$进行广播的两条链路，设$cSet(ru)$表示使用分配频率单元 的蜂窝链路集合，那么此时基站更新$dLink_m$的$cNborConfirmSe{t_m}$为：
	
	\begin{equation}\label{eq2.1}
	dNborC_m = dNborC_m \cup cSet(ru)
	\end{equation}
	
	如果$dLink_m$上报$ru$的类型为蜂窝频率类型$\mathcal{N}_{c0}$，则无需进行任何操作。
	...
	3）遍历每一个D2D链路的集合列表$List < dNborSet_{p}{ > _k}$，如果列表元素$dNborSet_{p}(m)$包含了$dLinkSet_{out}$中的$dLink$，则将该D2D链路集合$从列表中移除。
	...
	本文算法使用冲突权重$confic{t_{weight}}$来表示用来描述链路之间的广播冲突情况，定义两条D2D链路$dLink_m$和$dLink_n$的冲突权重为$confic{t_{weight}}(m,n)$，其中$confic{t_{weight}}(m,n)$的含义是根据现有的干扰图信息，同时与$dLink_m$和$dLink_n$的T端存在干扰关系的R端数量，包含蜂窝链路的R端和D2D链路的R端。这样根据D2D链路两两之间的冲突权重就构建出了小区的冲突权重表。
	用$isConfict(m,n,k)$表示$dLink_m$，$dLink_n$的广播在$dLink_k$的冲突情况。定义$dLink_m$，$dLink_n$与$dLink_k$的$isConfict(m,n,k)$：
