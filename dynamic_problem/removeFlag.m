...
Mpool = [1,2];
data = cell(length(Mpool),1);
u_frame_D2D = 10;  %每frame进入/离开的D2D用户个数
u_frame_cellular = 100;                      %每frame进入/离开的cellular link用户个数
for M_step = 1:length(Mpool)
    M = Mpool(M_step);
    r_cell = 300;                        %小区半径
    QD2D = 500;                          %D2D链路对数
    ...
    for i = 1:QD2D
        X_Yr(i,1)=D2D_loc(i,1)+D_D2D(i)*cos( A_D2D(i));
        X_Yr(i,2)=D2D_loc(i,2)+D_D2D(i)*sin( A_D2D(i));
    end
    ...
    for slot = 0:maxframe-1
        ...
        for i = QCellular+1:Q
            if D2D_loc(i-QCellular,1)^2+D2D_loc(i-QCellular,2)^2 < (d_broadcast(i))^2 && i~=j
        ...
        data_pp = [data_pp pp]
        slot = slot + 1;
        %save data_pp_30,'data_pp';
%         if pp==0
%             data_pp = [data_pp,zeros(1,400-length(data_pp))];
%             break;
%         end


    end
    data{M_step,1} = data_pp
end
save(mfilename,'data');
toc;
